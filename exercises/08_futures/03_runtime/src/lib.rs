...
{
    let reply: Arc<T> = Arc::new(reply);
    let handle1 = tokio::spawn(handle(first, Arc::clone(&reply)));
    let handle2 = tokio::spawn(handle(second, reply));

    let _ = tokio::join!(handle1, handle2);
}

async fn handle<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
