...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::InvalidTitle("Title cannot be longer than 50 bytes".into()));
        }
        if description.is_empty() {
            return Err(TicketNewError::InvalidDescription("Description cannot be empty".into()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::InvalidDescription("Description cannot be longer than 500 bytes".into()));
        }
